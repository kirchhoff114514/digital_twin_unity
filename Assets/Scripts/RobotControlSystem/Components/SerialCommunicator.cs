...
public class SerialCommunicator : MonoBehaviour
{
    ...
    [Tooltip("查找串口时用于过滤的关键字，例如 'USB', 'CH340', 'CP210X' 等。留空则尝试所有串口。")]
    public string portNameKeyword = "USB"; // 可根据你的ESP32串口驱动调整
    ...
    // 移除了 gripperOpenAngle, gripperCloseAngle, gripperNoneAngle
    // 这些参数现在应由 MotionPlanner 或其他高层逻辑负责定义和管理，
    // 因为 SerialCommunicator 不再负责 GripperState 到角度的转换。
    ...
    private void TryConnectSerialPort()
    {
        ...
        {
            // 尝试匹配关键字，或如果没有关键字则尝试所有
            if (string.IsNullOrEmpty(portNameKeyword) || port.Contains(portNameKeyword))
            {
                foundPort = port;
                break;
            }
        ...
        if (foundPort != null)
        {
            _currentPortName = foundPort;
            try
            {
                _serialPort = new SerialPort(_currentPortName, baudRate);
                _serialPort.ReadTimeout = receiveTimeoutMs; // 设置读取超时时间，防止阻塞
                _serialPort.WriteTimeout = 100; // 设置写入超时时间
                _serialPort.Open();
                _isConnected = true;
            ...
            {
                Debug.LogError($"SerialCommunicator: 连接串口 {_currentPortName} 失败: {e.Message}", this);
                _isConnected = false;
                // 如果连接失败，确保线程没有启动
        ...
        {
            Debug.LogWarning($"SerialCommunicator: 未找到包含 '{portNameKeyword}' 关键字的串口。", this);
        }
    }
    ...
    public void SendDesiredAnglesToHardware(float[] jointAngles, float desiredGripperAngle)
    {
        ...
        // **修改点 3：直接使用传入的 desiredGripperAngle**
        dataPart += $"J{robotDOF + 1}:{desiredGripperAngle:F1};"; // J6为夹爪目标角度

        // 计算 CRC8
        byte crc = CalcCRC8(dataPart);
        string packetToSend = $"{dataPart}CRC:{crc:X2} "; // :X2 将字节格式化为两位十六进制，并以空格结束
    ...
    private void ProcessReceivedPacket(string packet)
    {
        ...
        for (int i = 0; i < robotDOF; i++)
        {
            if (float.TryParse(match.Groups[i + 1].Value, out float angle))
            {
                ...
                dataPartForCrc += $"J{i + 1}:{angle:F1};"; // 重新构建，确保和发送端格式一致
            }
            else
            {
        ...
        if (float.TryParse(match.Groups[robotDOF + 1].Value, out float gripperA))
        {
            ...
            dataPartForCrc += $"J{robotDOF + 1}:{gripperA:F1};"; // 重新构建 J6
        }
        else
        {
        ...
        dataPartForCrc = dataPartForCrc.TrimEnd(';'); // 移除末尾分号以便CRC计算
