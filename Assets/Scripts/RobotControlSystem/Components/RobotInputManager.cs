...
public class RobotInputManager : MonoBehaviour
{
    ...
    private void HandleKeyboardInput()
    {
        ...
        if (Input.GetKey(KeyCode.I)) moveDelta += Vector3.forward; // Z-axis positive (forward)
        if (Input.GetKey(KeyCode.K)) moveDelta += Vector3.back;    // Z-axis negative (backward)
        if (Input.GetKey(KeyCode.M)) moveDelta += Vector3.left;    // X-axis negative (left)
        if (Input.GetKey(KeyCode.N)) moveDelta += Vector3.right;   // X-axis positive (right)
        if (Input.GetKey(KeyCode.L)) moveDelta += Vector3.up;  // Y-axis positive (up)
        if (Input.GetKey(KeyCode.J)) moveDelta += Vector3.down; // Y-axis negative (down)
        ...
        if (Input.GetKey(KeyCode.Y)) rotateDelta.x += 1; // Pitch up
        if (Input.GetKey(KeyCode.H)) rotateDelta.x -= 1; // Pitch down
        // Roll (around Z-axis): Use U/O
        if (Input.GetKey(KeyCode.U)) rotateDelta.z += 1; // Roll clockwise
        if (Input.GetKey(KeyCode.O)) rotateDelta.z -= 1; // Roll counter-clockwise
        ...
        if (rotateDelta != Vector3.zero)
        {
            ...
            newEulerAngles.y += rotateDelta.z * keyboardRotateSpeed * Time.deltaTime; // Roll
            ...
            newEulerAngles.y = Mathf.Repeat(newEulerAngles.z + 180, 360) - 180; // Clamp Roll to -180 to 180
